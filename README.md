...
**Host bind is loopback only** (`127.0.0.1:8080`). Do not publish `:8080` on `0.0.0.0`. Sidecar/harness listen on `0.0.0.0` **inside the compose network**; that is not a host bind. Plaintext HTTP is allowed only with `SOLUM_ALLOW_PLAINTEXT_HTTP=1` (dev-local). This demo is **not TLS**. `make up` writes a random `SOLUM_SIDECAR_TOKEN` to `.env` (gitignored). There is no shared default token in the compose file. nginx injects `X-Solum-Sidecar-Token`; the dashboard JS does not contain it.
