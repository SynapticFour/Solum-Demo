...
| **GitHub Actions** | Pin `actions/checkout` to a full commit SHA with a `# v4.x` comment so Dependabot can bump it. |
...
Weekly: Dependabot (`docker` + `github-actions`) plus [MONTHLY-DEPENDENCY-HYGIENE](https://github.com/SynapticFour/synapticfour-infra/blob/main/docs/MONTHLY-DEPENDENCY-HYGIENE.md) when that repo is in scope.
